...
class HRViTAttention(nn.Module):
    ...
    def _cross_shaped_attention(
        ...
        # B, H, h, bW, d = attn.shape
        # b = ws
        # W = qwe / b
        if horizontal:
            B1, H1, h1, bW1, d1 = attn.shape
            b1 = ws
            W1 = int(bW1 / b1)
            attn = attn.reshape(B1, int(H1*b1), W1, int(h1*d1))
        else:
            B1, W1, h1, bH1, d1 = attn.shape
            b1 = ws
            H1 = int(bH1 / b1)
            attn = attn.reshape(B1, H1, int(W1*b1), int(h1*d1))
